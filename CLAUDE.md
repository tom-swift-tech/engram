...
│   ├── mcp-tools.ts             ← MCP tool definitions (10 tools: retain/recall/reflect/extract/forget/supersede/session/queue_stats/requeue_failed/embed)
...
│   ├── mcp-server.test.ts
│   ├── cli.test.ts               ← engram CLI: per-subcommand happy path, --json contract, stdin, exit codes (24 tests)
...
│       └── tests/                ← 104 tests (adapter, scheduling, auto-retain, startup-recall, session-bridge, smoke + built-dist)
├── skills/
│   ├── engram.md                  ← portable agent skill (covers all 10 MCP tools)
