...
│   ├── reflect.ts               ← scheduled learning engine + prompt templates + reflectCatchUp() backlog-drain runner (D5) + opinion gates, belief journal, counter-evidence pass & falsifier weakening (issue #38)
...
├── tests/                        ← TS suites incl. aql-* cross-process (612 tests via npm test; +129 from integrations/pi, +67 from tools/openclaw-import)
│   ├── helpers.ts
│   ├── retain.test.ts
│   ├── retain-gate.test.ts
│   ├── recall.test.ts
