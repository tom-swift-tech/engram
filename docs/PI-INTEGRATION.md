...
On a genuinely fresh session, the **very next** `before_agent_start` recalls against the user's first prompt and prepends the formatted result to the system prompt as starting context. "Genuinely fresh" is `isFreshSessionStart(reason, priorMessageCount)` in `adapter.ts`: `reason: 'new'` always qualifies, and `reason: 'startup'` qualifies only when there are zero prior `type: 'message'` session entries. Both halves of that check exist because of a real gap found during live validation against an actual Pi runtime: every initial process launch — interactive `pi` or non-interactive `pi -p` — reports `reason: 'startup'`, never `'new'` (`'new'` only fires for an explicit mid-process session switch), and Pi always appends bookkeeping entries (`model_change`, `thinking_level_change`) before `session_start` fires, so a raw entry count is never zero even for a blank slate. Sessions continued via `--continue`/`--resume`/`--session` (which do have prior `message` entries) correctly fall through as non-fresh — already covered by the working-memory session bridge, not startup recall.
